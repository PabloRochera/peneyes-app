<section class="space-y-6 bg-white p-6 rounded-xl shadow-lg border border-gray-200">
    ...
    <x-danger-button
        ...
        class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg transition ease-in-out duration-150"
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <div class="mt-6">
                ...
                <x-text-input id="password" name="password" type="password" class="mt-1 block w-full border border-gray-300 rounded-lg p-2 shadow-sm" placeholder="{{ __('Password') }}" />
                <x-input-error :messages="$errors->userDeletion->get('password')" class="mt-2" />
            </div>
            <div class="mt-6 flex justify-end space-x-4">
                <x-secondary-button x-on:click="$dispatch('close')" class="bg-gray-500 text-white py-2 px-4 rounded-lg hover:bg-gray-600 transition">{{ __('Cancel') }}</x-secondary-button>
                <x-danger-button class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition">{{ __('Delete Account') }}</x-danger-button>
            </div>
        </form>
    </x-modal>
</section>
